...
function PolygonCreator(map, strokeColor, strokeOpacity, strokeWeight, fillColor, fillOpacity) { //
    this.map = map;
...
function Line(listOfDots, map) {
    ...
    if (this.listOfDots.length > 1) {
        ...
        jQuery('#showData').val(this.coords);
...
function Polygon(listOfDots, map, pen, color) {
    ...
    this.polygonObj = new google.maps.Polygon({paths: this.coords, editable:true, strokeColor: "#FF0000", strokeOpacity: 0.8, strokeWeight: 2, fillColor: "#FF0000", fillOpacity: 0.35, map: this.map});
    jQuery('#showData').val(this.coords);

    var pathPolygon = this.polygonObj.getPath();
